...
#include <DHT.h> // Pastikan Anda sudah menginstal library DHT
...
#define TELEMETRY_PROP_NAME_PPM "PPM"
...
static float get_ldr_value() {
  return analogRead(ldrPin);
}

static void get_ultrasonic_values(float* Distance) {
    ...
    size_t* payload_buffer_length)
{
  ...
  az_span json_span;
  float Illuminance,Distance,PPM;


  // Membaca data dari sensor
  Illuminance = get_ldr_value();
  get_ultrasonic_values(&Distance);
...
// Tambahkan data LDR
  ...
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding LDR property value to telemetry payload.");
 ...
 rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("Distance"));
  ...
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding Distance property value to telemetry payload.");

rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("PPM"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding LDR property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, PPM, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding LDR property value to telemetry payload.");
